...
BOOL AutoPowerSave    = TRUE;   // Variable to enable/disable the Auto Power Save func, TRUE = 1 (voir main.h)*/
/* #define BOOL    char (voir main.h) */
volatile BOOL gKeyClickStatus  = FALSE;  // Variable to enable/disable keyclick, FALSE = 0 (voir main.h)*/
...
//  unsigned char state, nextstate;
    ...
    PGM_P statetext; /* statetext est un pointeur d'une string (voir pgmspace.h) */
    char (*pStateFunc)(char); /* pointeur de fonction dont le paramètre est un char */
    char input;
    uint8_t i, j; // char i; /* il s'agit d'un unsigned char (voir stdint.h) */
    char buttons;
    char last_buttons;
    ...
    state = ST_AVRBF; /* state = 10 (voir main.h)*/
    nextstate = ST_AVRBF; /* nextstate = 10 */
    statetext = MT_AVRBF;
	 /* statetext = {'A','V','R',' ','B','U','T','T','E','R','F','L','Y',' ','G'+0x80,'C'+0x80,'C'+0x80,'\0'};
	 (voir menu.h)*/
    pStateFunc = NULL; /* #define NULL    0 (voir main.h)*/
    ...
    for (;;)            // Main loop
    {
        ...
        {
            // Plain menu text
            if (statetext) /* si Statetext non vide */
            {
                LCD_puts_f(statetext, 1); /* affichage du string statetext contenu dans la mémoire FLASH
				/*le paramètre 1 = scrollmode et ne semble pas être utilisé dans la fonction (voir LCD_functions.c*/ 
                LCD_Colon(0); /*Enables the colon if TRUE, disable if FALSE*/
                statetext = NULL;
            }
 
            input = getkey();           /* Read buttons, cette fonction désactive toutes les interruptions (voir button.c)*/
            ...
            {
                state = nextstate; /* l'état est maintenant le nouveau état de la séquence définie dans menu.h*/
...
unsigned char StateMachine(char state, unsigned char stimuli)
{
    ...
    for (i=0; ( j=pgm_read_byte(&menu_nextstate[i].state) ); i++ )
    {
             ...
             pgm_read_byte(&menu_nextstate[i].input) == stimuli)
        {
            ...
            nextstate = pgm_read_byte(&menu_nextstate[i].nextstate); /* pourquoi pgm_read_byte, pourquoi & */
            break;
        }
    }
